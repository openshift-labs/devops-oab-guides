...
cat <<'EOF' > Jenkinsfile.prod
def namespace, releaseTag, prodCluster, prodProject, prodUser, prodToken
...
pipeline {
  ...
  stages {
    stage('Select PROD Version') {
      steps {
        script {
          openshift.withCluster() {
            namespace = openshift.project()
            prodCluster = env.PROD_CLUSTER.replace("https://","insecure://")
...
oc patch bc coolstore-create-prod  -p '{"spec":{"strategy":{"jenkinsPipelineStrategy":{"jenkinsfilePath":"Jenkinsfile.prod"}}}}'
oc set env bc/coolstore-create-prod PROD_REGISTRY={{?}} PROD_CLUSTER={{?}} 

...
WARNING: If you have succesfully deployed Jenkins, do NOT execute this command.
...
Now, clone the `cd-infra` git repository into your workspace as we will add the neccesary pipelines:
...
pipeline {
  ...
  stages {
    stage('Build JAR') {
      steps {
        git url: 'http://{{GIT_HOSTNAME}}/developer/catalog-spring-boot.git'
...
First, let's create the Pipeline's BuildConfig definition:

[source,shell,role=copypaste]
----
cat <<'EOF' > /tmp/pipeline-catalog-build.yml
...
spec:
  ...
  source:
    git:
      ref: master
      uri: "http://{{GIT_HOSTNAME}}/developer/cd-infra.git"
    type: Git
  strategy:
...
NOTE: At this moment you can not create the pipeline definition using the OpenShift UI

And now let's add this Pipeline to OpenShift:

[source,shell,role=copypaste]
----
oc create -f /tmp/pipeline-catalog-build.yml -n cicd{{PROJECT_SUFFIX}}
----

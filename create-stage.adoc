...
part of the release process we update the version in the maven 
http://{{ GIT_HOSTNAME }}/{{ GIT_USER }}/catalog-spring-boot/src/master/pom.xml#L6[`pom.xml`], so this new code will 
be also deployed into our *CoolStore DEV* environment.
